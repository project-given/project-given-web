...
>
	<div class="group relative flex h-full w-full flex-col">
		...
		<div transition:fade class="flex min-h-[200px] flex-col justify-between p-6">
			<div class="text-lg text-blue-0">{title}</div>
			<div class="text-end text-sm text-slate-500">{createdAt}</div>
		</div>
	</div>
</div>
<!-- class="absolute bottom-0 flex flex-row items-end justify-start w-full h-20 p-5 bg-opacity-40 bg-gradient-to-t from-black pl-7" -->
...
>
	<div class="relative flex-grow w-full overflow-hidden">
		...
		/>
		{#if hovered}
			<div transition:fade class="absolute top-0 z-10 w-full h-full p-8 bg-black opacity-70">
